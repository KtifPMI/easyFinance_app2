...
import { FinancialEvent, Goal } from '../../types';

type Nav = NativeStackNavigationProp<RootStackParamList & HomeStackParamList>;

export function HomeScreen() {
  const navigation = useNavigation<Nav>();
  const { accounts, operations, budgets, goals, events, recommendations, isLoading, loaded, loadAll } = useFinanceStore();
  ...
  const finHealth = calcFinHealth(accounts, operations, budgets);
  ...
  const expensePercent = totalPlanned > 0 ? (totalSpent / totalPlanned) * 100 : 0;
    ...
    <View style={{ flex: 1, backgroundColor: colors.background }}>
      <View style={styles.scroll} >
        <ScrollContent
          ...
          income={income} expense={expense} capital={capital}
          expensePercent={expensePercent}
          upcomingEvents={upcomingEvents} goals={goals}
          recommendations={recommendations}
          daysUntilEmpty={daysUntilEmpty} cashflowWarning={cashflowWarning}
          userName={user?.name} isLoading={isLoading} loadAll={loadAll}
          finHealth={finHealth}
        />
      </View>
    </View>
...
function ScrollContent({ navigation, totalBalance, income, expense, capital, expensePercent, upcomingEvents, goals, recommendations, daysUntilEmpty, cashflowWarning, userName, isLoading, loadAll, finHealth }: {
  navigation: Nav; totalBalance: number; income: number; expense: number; capital: number;
  expensePercent: number; upcomingEvents: FinancialEvent[]; goals: Goal[];
  recommendations: import('../../types').Recommendation[];
  daysUntilEmpty: number | null; cashflowWarning: boolean; userName?: string;
  isLoading: boolean; loadAll: () => Promise<void>;
      ...
      {cashflowWarning && daysUntilEmpty !== null && (
        <Pressable style={styles.cashflowAlert} onPress={() => navigation.navigate('MoreTab' as never)}>
          ...
          <View style={{ flex: 1, marginLeft: spacing.sm }}>
            <Text style={styles.cashflowAlertTitle}>Кассовый разрыв через {daysUntilEmpty} дней</Text>
            <Text style={styles.cashflowAlertText}>При текущем темпе трат баланс уйдёт в ноль.</Text>
          </View>
        </Pressable>
      )}
            ...
            return (
              <Pressable key={goal.id} style={styles.goalRow} onPress={() => navigation.navigate('GoalDetail', { goalId: goal.id })}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={styles.goalAmounts}>{formatMoney(goal.currentAmount)} из {formatMoney(goal.targetAmount)}</Text>
                </View>
              </Pressable>
            );
          })
...
const styles = StyleSheet.create({
  ...
  balanceLabel: { ...typography.body, color: '#E8F5E9' },
  balanceValue: { ...typography.h1, color: colors.white, marginTop: spacing.xs, marginBottom: spacing.lg },
  row: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  rowItem: { flex: 1 },
  rowHeader: { flexDirection: 'row', alignItems: 'center', gap: 4, marginBottom: 4 },
  rowLabel: { ...typography.caption, color: '#E8F5E9' },
  rowValue: { ...typography.h3, color: colors.white },
  rowValueBold: { ...typography.h3, color: colors.white },
  ...
  budgetText: { ...typography.body, color: colors.text },
  budgetPercent: { ...typography.bodyBold, color: colors.text },
  warningText: { ...typography.caption, color: colors.warning, marginTop: spacing.sm },
  emptyText: { ...typography.body, color: colors.textSecondary },
  eventRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: spacing.sm },
  eventIcon: { width: 36, height: 36, borderRadius: 18, backgroundColor: colors.background, alignItems: 'center', justifyContent: 'center', marginRight: spacing.sm },
  eventTitle: { ...typography.body, color: colors.text },
  eventDate: { ...typography.small, color: colors.textSecondary },
  eventAmount: { ...typography.bodyBold },
  goalRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: spacing.sm, gap: spacing.md },
  goalIcon: { width: 40, height: 40, borderRadius: 20, alignItems: 'center', justifyContent: 'center' },
  goalTitle: { ...typography.bodyBold, color: colors.text, marginBottom: 4 },
  goalAmounts: { ...typography.small, color: colors.textSecondary, marginTop: 4 },
  recRow: { flexDirection: 'row', alignItems: 'flex-start', paddingVertical: spacing.sm, borderBottomWidth: 1, borderBottomColor: colors.border },
  recTitle: { ...typography.bodyBold, color: colors.text, marginBottom: 2 },
  recDesc: { ...typography.small, color: colors.textSecondary },
  cashflowAlert: { flexDirection: 'row', alignItems: 'flex-start', backgroundColor: colors.expense, borderRadius: 12, padding: spacing.md, marginTop: spacing.lg },
  ...
  adviceCard: { marginTop: spacing.lg, marginBottom: spacing.md },
});
