    ...
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        <View style={styles.typeRow}>
          {TYPE_OPTIONS.map((opt) => (
            <Pressable
              ...
              onPress={() => { setType(opt.key); setCategoryId(undefined); }}
            >
        ...
        <View style={styles.amountWrapper}>
          <TextInput
            style={styles.amountInput} value={amount}
            onChangeText={(v) => setAmount(v.replace(/[^0-9]/g, ''))}
            keyboardType="number-pad" placeholder="0" placeholderTextColor={colors.border} autoFocus
          />
            ...
            <View style={styles.categoryGrid}>
              {filteredCategories.map((cat) => (
                <Pressable key={cat.id} style={styles.categoryItem} onPress={() => setCategoryId(cat.id)}>
                  <View style={[styles.categoryIcon, { backgroundColor: cat.color + '22' }, categoryId === cat.id && { borderWidth: 2, borderColor: cat.color }]}>
        ...
        <TextInput
          ...
          placeholder="Например: Пятёрочка" placeholderTextColor={colors.textSecondary}
        />
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  typeButton: { flex: 1, paddingVertical: spacing.sm, borderRadius: radius.md, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.card, alignItems: 'center' },
  typeLabel: { ...typography.bodyBold, color: colors.textSecondary },
  amountWrapper: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginBottom: spacing.xl },
  amountInput: { fontSize: 44, fontWeight: '700', color: colors.text, minWidth: 80, textAlign: 'right' },
  currency: { fontSize: 32, fontWeight: '700', color: colors.textSecondary, marginLeft: spacing.sm },
  sectionLabel: { ...typography.caption, color: colors.textSecondary, marginBottom: spacing.sm, marginTop: spacing.sm },
  ...
  categoryIcon: { width: 52, height: 52, borderRadius: 26, alignItems: 'center', justifyContent: 'center', marginBottom: spacing.xs },
  categoryLabel: { ...typography.small, color: colors.text, textAlign: 'center' },
  commentInput: { height: 50, borderRadius: radius.md, borderWidth: 1, borderColor: colors.border, paddingHorizontal: spacing.md, backgroundColor: colors.card, ...typography.body, color: colors.text },
